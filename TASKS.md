...
- [ ] **1.11** `up`: when `--identity` set → `provider.IsAuthenticated` (re-auth if stale) → `provider.Credential` → check `harness.Supports(cred.Vendor)` → `broker.Store` → `IssueHandle` → `harness.Env(broker.Addr, handle)` → fold env + `Provisions()` into the spec. **Delete the old `CLAUDE_CODE_OAUTH_TOKEN` injection**, and once `applyIdentity` no longer calls it, remove `Provider.Materialize` from the interface + `FakeProvider` + anthropic (orphaned after 1.6). Test with a fake broker seam + fake provider/harness (assert: handle in env, real secret NOT in env). Not end-to-end runnable until 1.12 serves the broker.
- [ ] **1.12** `up` lifecycle (Phase-1, up-scoped): serve the broker → issue handle → attach; revoke + `Stop` when the attached session ends. **`--detach` + `--identity` → error** ("detached identity pods need poddled — Phase 2"). Cross-process revoke-on-`down` is deferred to the poddled phase (the broker dies with the `up` process in Phase 1). Test the up-scoped lifecycle.
