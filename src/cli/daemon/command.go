...
func NewCmd() *cobra.Command {
	var gatewayBind, egress, socket string
	c := &cobra.Command{
		Use:    "daemon",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			defer stop()
			return poddled.Serve(ctx, socket, gatewayBind, egress)
		},
	}
	c.Flags().StringVar(&gatewayBind, "gateway-bind", "0.0.0.0:0", "gateway bind address pods reach")
